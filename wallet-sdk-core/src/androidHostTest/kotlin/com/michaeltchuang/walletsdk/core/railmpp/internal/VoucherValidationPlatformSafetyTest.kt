...
class VoucherValidationPlatformSafetyTest {
    ...
    fun `EXPECT validation to only compile, build, and simulate WHEN checking both platform sources for wallet-signing calls`() {
        ...
        for (platform in listOf("android", "ios")) {
            ...
            val validation = source.substringAfter("internal actual suspend fun validateLogicSigSettlementInternal(")
            ...
            assertTrue(
                if (platform == "android") "\"/v2/transactions/simulate\"" in validation
                else "syncSimulateTransactionWithAlgodUrl(" in validation,
            )
            for (forbidden in listOf(
                "signTransactionBytes", "signTransactionsBytes", "signTxnGroup", "broadcast",
                "ensureLogicSigSetup(", "fundLogicSigIfNeeded(", "executeLogicSigSettlement(",
                "funderSigner.authorizedSignerPublicKey", "funderSigner.signerType", "buildAppCallTxn",
            )) {
