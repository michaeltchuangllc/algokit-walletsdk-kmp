...
object MppPayments {
    ...
    fun decodeSignedTransaction(bytes: ByteArray): Any? =
        runCatching { decodeMsgPackAny(bytes) }.onFailure {
    ...
    fun createVoucherJson(
        ...
        val sigField = if (!signatureBase64.isNullOrBlank()) ""","signature":"$signatureBase64"""" else ""
        return """{"reference":"liquid:payment:voucher","id":"$sessionId","appId":$appId,"viewer":"$viewerAddress","viewerPublicKey":"$viewerPkBase64","creator":"$creatorAddress","blocksWatched":$blocksConsumed,"costPerBlockMicroUsdc":$COST_PER_BLOCK_MICRO_USDC,"totalAmountClaimedMicroUsdc":$totalAmountUsed,"remainingMicroUsdc":$remainingMicroUsdc$sigField}"""
    }
    ...
    fun getRemainingBalanceFromSessionVaultByChannelId(
        ...
        val data = contractClient(appId = appId, algodUrl = algodUrl)
            .getSessionDynamicData(channelId, algodUrl).getOrThrow()
    ...
    suspend fun setAuthorizedSignerForSession(
        ...
        val result = contractClient(appId, algodUrl = algodUrl).setAuthorizedSignerPublicKey(
            signer, channelId, authorizedSignerPublicKey, algodUrl,
        )
        result
    ...
    suspend fun updateVoucherOnChain(
        ...
        Napier.d("[VIEWER_UPDATE_VOUCHER_ATTEMPT] appId=$appId viewer=$viewerAddress host=$hostAddress claimedMicroUsdc=$totalAmountUsedMicroUsdc channelIdHash=$channelIdHash", tag = TAG)

        val result = contractClient(appId, algodUrl = algodUrl).updateVoucher(
            signer, channelId, totalAmountUsedMicroUsdc, signature, algodUrl,
        )
        result
            .onSuccess { Napier.d("[VIEWER_UPDATE_VOUCHER_OK] txId=$it channelIdHash=$channelIdHash", tag = TAG) }
            .onFailure { throwable ->
                val err = throwable.message.orEmpty()
                val isDuplicate = err.contains("pc=622", ignoreCase = true) &&
                    (err.contains("opcodes=dig 2", ignoreCase = true) || err.contains("Voucher not increasing", ignoreCase = true))
                if (isDuplicate) {
                    Napier.e("[VIEWER_UPDATE_VOUCHER_DUPLICATE_SKIP] channelIdHash=$channelIdHash reason=already_recorded_onchain", tag = TAG)
                } else {
    ...
    fun getSessionProgressSnapshotFromVault(
        ...
        val data = getSessionDynamicDataFromVault(viewerAddress, hostAddress, appId, algodUrl, authorizedSignerPublicKey)
            ?: return null
    ...
    fun getSessionDynamicDataFromVault(
        ...
        val signerCandidates = authorizedSignerPublicKey?.takeIf { it.isNotEmpty() }?.let { listOf(it) }
            ?: listOf(viewerPk, hostPk)
        val channelIdCandidates = signerCandidates
    ...
    fun getSessionDynamicDataFromVaultByChannelId(
        ...
        algodUrl: String = TESTNET_ALGOD_URL,
        logContext: String? = null,
    ): SessionDynamicData? = runCatching {
    ...
    ): ByteArray = contractClient(usdcAssetId = usdcAssetId).deriveChannelId(
    ...
    fun buildClaimMessage(appId: Long, channelId: ByteArray, totalAmountClaimedMicroUsdc: Long): ByteArray =
        encodeUint64(appId) + channelId + encodeUint64(totalAmountClaimedMicroUsdc) + "settle".encodeToByteArray()
    ...
    fun buildClaimMessage(
        ...
        authorizedSignerPublicKey: ByteArray = decodeAlgorandAddressPublicKey(viewerAddress),
    ): ByteArray = buildClaimMessage(appId, deriveChannelId(viewerAddress, hostAddress, authorizedSignerPublicKey), totalAmountClaimedMicroUsdc)

    @OptIn(ExperimentalEncodingApi::class)
    fun serializeVoucherSignature(signature: ByteArray): String = Base64.encode(signature)

    fun buildClaimMessageHashHex(appId: Long, channelId: ByteArray, totalAmountClaimedMicroUsdc: Long): String =
        hashHex(buildClaimMessage(appId, channelId, totalAmountClaimedMicroUsdc))
    ...
    fun buildClaimMessageHashHex(
        ...
        hostAddress: String,
        totalAmountClaimedMicroUsdc: Long,
    ): String = buildClaimMessageHashHex(
        appId,
    ...
    fun hashHex(bytes: ByteArray): String = sha256(bytes).joinToString("") {
...
sealed class MppPaymentVerificationResult {
    ...
    data class Invalid(val reason: String) : MppPaymentVerificationResult()
}
