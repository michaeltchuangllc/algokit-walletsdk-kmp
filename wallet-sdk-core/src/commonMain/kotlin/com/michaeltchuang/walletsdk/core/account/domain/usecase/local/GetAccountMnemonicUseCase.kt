...
internal class GetAccountMnemonicUseCase(
    ...
    override suspend fun invoke(address: String): WalletSdkResult<AccountMnemonic> = when (val account = getLocalAccount(address)) {
        is LocalAccount.Algo25 -> mnemonic(getMnemonicFromAlgo25SecretKey(getAlgo25SecretKey(address) ?: return WalletSdkResult.Error(IllegalArgumentException())), AccountMnemonic.AccountType.Algo25)
    ...
    private suspend fun hdMnemonic(seedId: Int, type: AccountMnemonic.AccountType): WalletSdkResult<AccountMnemonic> {
    ...
    private fun mnemonic(value: String?, type: AccountMnemonic.AccountType): WalletSdkResult<AccountMnemonic> = if (value.isNullOrBlank()) WalletSdkResult.Error(IllegalArgumentException()) else WalletSdkResult.Success(AccountMnemonic(value.splitMnemonic(), type))
}
