...
internal class DefaultPasskeyRepository(
    ...
    override fun getAllPasskeysAsFlow(): Flow<List<Passkey>> =
        passkeySiteDao.getPasskeysAsFlow().map { siteWithPasskeysQueries ->
            siteWithPasskeysQueries
                .map { siteWithPasskeys ->
    ...
    override suspend fun getCredentialIdByAlgoAddress(algoAddress: String): String? {
        return passkeyDao.getCredentialIdByAlgoAddress(algoAddress)
    }
    ...
    override suspend fun setPasskeyLastUsedTime(
        credId: String,
        lastUsed: Long,
    ) {
