                ...
                scope.launch {
                    ...
                    peerClient!!.createPeerConnection(
                        ...
                        { iceCandidate ->
                            Log.d(
                                TAG,
                                "onIce${clientType.replaceFirstChar {
                                    if (it.isLowerCase()) {
                                        it.titlecase(
                                            Locale.ROOT,
                                        )
        ...
        override suspend fun signal(type: String): SessionDescription =
            suspendCoroutine { continuation ->
                this.socket!!.once("$type-description") {
                    ...
                    Log.d(
                        TAG,
                        "signal.on${type.replaceFirstChar {
                            if (it.isLowerCase()) {
                                it.titlecase(
                                    Locale.ROOT,
                                )
            ...
            suspendCoroutine { continuation ->
                ...
                socket!!.emit(
                    ...
                    Ack { args: Array<Any> ->
                        ...
                        continuation.resume(LinkMessage.Companion.fromJson(response.toString()))
                    },
                )
            }
