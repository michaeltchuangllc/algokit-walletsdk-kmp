...
class MppPaymentViewerSingleFlightTest {
    ...
    fun manualTopUpBlocksPollingAndQueuedConsentUntilConfirmed() = scenario {
        ...
        val topUp = async {
            manager.topUpViewerSessionVault(
                ...
                fund = { deposits++; transaction.await() },
    ...
    private class Fixture(val testScope: TestScope) : CoroutineScope by testScope {
        ...
        val params = MppPaymentViewerManager.StartParams(
            ...
            requestMppConsent = { prompts++; requestConsent(it) },
        ...
        fun start() {
            ...
            coEvery { balanceUseCase(any()) } coAnswers { reads++; readBalance() }
            coEvery { MppPayments.getSessionDynamicDataFromVault(any()) } returns
                MppPayments.SessionDynamicData(100, 0, 0, 1)
            coEvery { MppPayments.topUpSessionVault(any(), any()) } coAnswers { deposits++; fund() }
        ...
        fun refresh() = manager.startViewerOnChainRefresh(
            scope = testScope,
    ...
    private class Channel : RtcDataChannel {
        ...
        fun gate() = receive(buildJsonObject {
            put("type", DCMessageType.SEGMENT_REJECTED.value)
            put("payload", buildJsonObject { put("reason", "exhausted") })
        })

        fun request() = receive(buildJsonObject {
            put("type", DCMessageType.SEGMENT_REQUEST.value)
            put("payload", Json.encodeToJsonElement(PaymentRequest(
                id = "request", sessionId = "session", segmentIndex = 0, amount = "10",
                asset = "USDC", network = MppNetworks.ALGORAND_TESTNET, payTo = "creator",
                ttl = 60, nonce = "nonce",
